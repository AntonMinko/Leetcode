    ...
    public class MinimumCostForTickets
    {
        public int MincostTickets(int[] days, int[] costs)
        {
            ...
            int MinCost(int pos)
            {
                ...
                int GetNextPos(int incr)
                {
                    int day = days[pos];
                    int nextPos = pos + 1;
                    while (nextPos < days.Length && days[nextPos] < day + incr)
                    {
                        nextPos++;
                    }
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    {
                        case 0:
                            remainingCost = MinCost(GetNextPos(1));
                            break;
                        case 1:
                            remainingCost = MinCost(GetNextPos(7));
                            break;
                        default:
                            remainingCost = MinCost(GetNextPos(30));
                            break;
                    }
                    min = Math.Min(min, remainingCost + costs[i]);
                }
